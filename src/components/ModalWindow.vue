<template>
  <div class="modal-wrap">
    <div class="image-form">
      <!-- <span class="title">Choose the best 🐾 of your photos</span> -->
      <label v-if="getNoFile" for="img" class="choose-image">
        <span class="plus">+</span>
      </label>
      <transition name="popup-delay">
        <alert-box v-if="getOverLimit" >
          Oops, the file size must be 5MB or less!
        </alert-box>
      </transition>
      <div ref="preview" id="preview-wrap">
        <label v-show="!getNoFile" for="img" class="change-image">
    ...
    <transition name="popup-2-delay">
      <div class="progress-wrap">
        <div class="items">
          <progress-box class="is-selected-a-photo current" :fileSelected="getFileSelected">
            Click above "+" area to select a photo
          </progress-box>
          <progress-box class="is-the-cat" :apiStatus="getApiCatStatus">
            We're checking to see if that's Nyanko<span class="emoji">🐱</span> in this picture...
          </progress-box>
          <!-- <progress-box class="is-exist-same-image" :apiStatus="getApiSameStatus" dur="1.1s">
            We're checking to see if the same photo is not yet in the database...
          </progress-box> -->
          <progress-box class="is-accepted" :apiStatus="getIsAccepted">
    ...
    <div class="accept-box">
      <label for="accept" @click="falseShowAlert(); enableSubmitButton()">
        <input type="checkbox" id="accept" v-model="isCheckedAccept">
    ...
    <transition name="popup">
      <alert-box v-show="getShowAlert" class="accept">
        Please accept privacy policy and license statement.
      </alert-box>
    </transition>
    <div class="btn-wrap">
      <button id="submit" @click="submit()" :disabled="getSubmitDisabled">Submit !</button>
...
import {apiKeyIsTheCat} from "@/utils/secret.js";
export default {
  mixins: [
    ModalClose,
  ],
  ...
  methods: {
    async onFileSelect() {
      ...
      if (isOverSizeLimit(file)) {
        this.$store.commit("trueOverLimit");
        this.$refs.fileInput.value = '';
        return;
      }
      this.$store.commit("falseOverLimit");
      this.$store.commit("falseNoFile");
      ...
      this.$store.commit("changeFileSelected", true);
      setTimeout(() => {
    ...
    submit() {
      // validation
      if (!this.isCheckedAccept) {
        this.$store.commit("trueShowAlert");
      }
  ...
  computed: {
    getNoFile() {
      return this.$store.state.noFile;
    },
    getOverLimit() {
      return this.$store.state.overLimit;
    },
    getShowAlert() {
      return this.$store.state.showAlert;
    },
    getFileSelected() {
      return this.$store.state.fileSelected;
    },
    getApiCatStatus() {
...
function drawBoundingBox(boundingBoxArray, target) {
  for (let box of boundingBoxArray) {
    ...
    boxElem.style.transition = "0.23s all ease-out";
    ...
    label.change-image {
      ...
      div.tooltip {
        ...
        &::before {
          ...
          border-top: solid 5.5px #0a0400ab;
          top: 21px;
          left: 50%;
